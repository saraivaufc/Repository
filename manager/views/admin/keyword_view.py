...
#from django.contrib.auth.decorators import login_required, permission_required
#@method_decorator(permission_required, 'manager.admin_keyword_list')

class KeywordListView(ListView):
	template_name = 'manager/admin/keyword/list.html'
...
class KeywordCreateView(CreateView):
	template_name = 'manager/admin/keyword/form.html'
	model = Keyword
	fields = ['name']
	success_url = reverse_lazy('manager:admin_keyword_list', kwargs={'page': 1})
...
class KeywordDeleteView(DeleteView):
	template_name = 'manager/admin/keyword/check_delete.html'
	model = Keyword
	success_url = reverse_lazy('manager:admin_keyword_list', kwargs={'page': 1})

class KeywordDetailView(DetailView):
	template_name = 'manager/admin/keyword/detail.html'
	model = Keyword
